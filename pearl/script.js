document.addEventListener("DOMContentLoaded", () => {
  ...
  const pearlLockup = document.querySelector(".hero-logo-lockup");
  if (document.documentElement.dataset.edition === "pearl-shimmer" && pearlLockup) {
    pearlLockup.addEventListener("pointermove", () => {
      pearlLockup.classList.add("is-shimmering");
    }, { passive: true });
    pearlLockup.addEventListener("pointerleave", () => {
      pearlLockup.classList.remove("is-shimmering");
    });
  }
    ...
    if (gl) {
      ...
      const fragmentSource = `
        ...
        void main() {
          ...
          vec3 signalRed = vec3(0.63, 0.72, 0.9);
  ...
  const drawMeshAnaconda = (now) => {
    ...
    for (let layer = 0; layer < layers; layer += 1) {
      ...
      visualContext.strokeStyle = layer === 2
        ? "rgba(64, 74, 102, 0.88)"
        : `rgba(64, 74, 102, ${0.15 + (1 - Math.abs(depth - 0.5) * 2) * 0.25})`;
  ...
  if (document.documentElement.dataset.edition === "pearl-shimmer") return;
  ...
  const surfaceMaterial = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        color += mix(vec3(0.05, 0.22, 0.85), vec3(0.68, 0.76, 0.92), uHigh)
          * edgeAura;
