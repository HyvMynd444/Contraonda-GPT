    ...
    if (gl) {
      ...
      const fragmentSource = `
        ...
        void main() {
          ...
          vec3 signalRed = vec3(1.0, 0.045, 0.11);
  ...
  const drawMeshAnaconda = (now) => {
    ...
    for (let layer = 0; layer < layers; layer += 1) {
      ...
      visualContext.strokeStyle = layer === 2
        ? "rgba(255, 45, 67, 0.88)"
        : `rgba(255, 45, 67, ${0.15 + (1 - Math.abs(depth - 0.5) * 2) * 0.25})`;
  ...
  const surfaceMaterial = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        vec3 normal = normalize(vec3(microGradient * 1.35, 1.0));
        ...
        color += electricBlue * blueDiffuse * 0.055;
        color += ultraviolet * blueSpecular * 0.07;
        color += acid * acidDiffuse * 0.026;
        color += mix(acid, prism, shimmer)
          * acidSpecular * (0.1 + uLevel * 0.16 + shimmer * 0.08);
        color += mix(electricBlue, acid, vHeight) * fresnel * 0.035;
        color += prism * shimmer * fresnel * (0.035 + uHigh * 0.1);
        color += mix(vec3(0.025, 0.34, 1.0), vec3(0.38, 0.92, 1.0), rollingBand)
          * edgeSignal * (0.28 + uLevel * 0.46);
        color += mix(vec3(0.05, 0.22, 0.85), vec3(1.0, 0.025, 0.075), uHigh)
          * edgeAura;
        ...
        float alpha = (0.11 + fresnel * 0.045 + blueSpecular * 0.028
          + acidSpecular * 0.035 + uLevel * 0.1
